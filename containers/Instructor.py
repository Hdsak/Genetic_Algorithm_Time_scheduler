...
class Tree:
    def __init__(self, tree):
        ...
        model.setHorizontalHeaderLabels(['ID', 'Свободен', 'Имя', 'Часы', 'Действия'])
        tree.setModel(model)
    ...
    def display(self):
        ...
        for instr in result:
            ...
            btnEdit = QtWidgets.QPushButton('Редактировать', frameEdit)
            btnEdit.clicked.connect(lambda state, id=instr[0]: self.edit(id))
            btnDelete = QtWidgets.QPushButton('Удалить', frameEdit)
    ...
    def delete(self, id):
        ...
        confirm.setText('Удалить отношение?')
        confirm.setWindowTitle('Подтвердить удаление')
