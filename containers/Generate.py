...
class Generate:
    ...
    def togglePause(self):
        self.toggleState()
        self.parent.btnPause.setText('Остановить генетический алгоритм' if self.running else 'Возобновить')
    ...
    def updateStatus(self, status):
        self.parent.lblStatus.setText('Статус: {}'.format(status))

    def updateDetails(self, details):
        self.parent.boxGen.setTitle('Поколение #{}'.format(details[0]))
        self.parent.lblPopulation.setText('Популяция: {}'.format(details[1]))
        self.parent.lblMutation.setText('Шанс мутации: {}%'.format(details[2]))
        self.parent.lblFitness.setText('Средняя приспособляемость: {}%'.format(details[3]))
        self.parent.lblPreviousFitness.setText('Предыдущее средняя приспособляемость: {}%'.format(details[4]))
        self.parent.lblHighestFitness.setText('Наивысшая приспособляемость: {}%'.format(details[5]))
        self.parent.lblLowestFitness.setText('Низшая приспособляемость: {}%'.format(details[6]))
    ...
    def updateTime(self):
        ...
        self.parent.lblTime.setText('Пройдено времени: {}'.format(self.time.toString('hh:mm:ss')))
    ...
    def stopOperation(self):
        ...
        if len(self.topChromosomes):
            self.parent.btnStop.setText('Показать результаты')
            ...
            self.parent.lblMemory.setText('Использовано память: Stopped')
            self.parent.lblStatus.setText('Статус: Stopped')
