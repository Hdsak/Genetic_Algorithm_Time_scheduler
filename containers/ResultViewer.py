...
class ResultViewer:
    ...
    def getLastResult(self):
        ...
        if result:
            ...
            messageBox.setText('Решение не сгенерировано')
    ...
    def parseResultDetails(self):
        ...
        self.parent.lblTime.setText('Время поколения: {}'.format(result['time']))
    ...
    def updateDetails(self, index):
        ...
        parent.lblFit.setText('Конечная приспособленность: {}%'.format(meta[0]))
        parent.lblSbj.setText('Тема размещения(Subject Placement): {}%'.format(meta[1][0]))
        parent.lblSecRest.setText('Section Rest: {}%'.format(meta[1][2]))
        parent.lblSecIdle.setText('Простой группы (Section Idle Time): {}%'.format(meta[1][4]))
        parent.lblInstrRest.setText('Перерыв преподавателя(Instructor Rest): {}%'.format(meta[1][3]))
        parent.lblInstrLoad.setText('Нагрузка преподователя(Instructor Load): {}%'.format(meta[1][6]))
        parent.lblLunch.setText('Обеденный перерыв: {}%'.format(meta[1][1]))
        parent.lblMeet.setText('Формат встречи (Meeting Pattern): {}%'.format(meta[1][5]))
